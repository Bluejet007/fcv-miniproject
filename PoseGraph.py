...
i_matrix = np.eye(4)
class Camera(object):
    def __init__(self, desc_dict, image: np.ndarray, count: np.ndarray):
        self.count = count
        self.count_inv = np.linalg.inv(self.count)

        self.pose = i_matrix
        self.h, self.w = image.shape[0:2]
        key_pts, self.descriptors = feat_mapping(image)
        self.key_pts = normalise(self.count_inv, key_pts)
